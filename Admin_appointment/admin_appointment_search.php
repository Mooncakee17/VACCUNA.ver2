...
include '../Homepage/config.php';
	...
	$search_by = "";
	if($searchBy == 1){
		$search_by = "appointment_status";
	}
	else if($searchBy == 2){
		$search_by = "for_reason";
	}

	if($searchBy == 1 || $searchBy == 2){
		$selectedValue = $_POST['selectedValue'];
		$select = mysqli_query($conn, "SELECT * FROM appointmenttable a
		LEFT JOIN vaccineinventory b ON a.vacid = b.vacid WHERE `$search_by` = '$selectedValue' ORDER BY appointment_status ASC") or die('query failed');
	}
	else if($searchBy == 3){
		$searchData = $_POST['searchData'];
		$select = mysqli_query($conn, "SELECT * FROM appointmenttable a
		LEFT JOIN vaccineinventory b ON a.vacid = b.vacid WHERE CONCAT(child_name, guardian_name,appt_id,vac_name) LIKE '%$searchData%' ORDER BY appointment_status ASC") or die('query failed');
	}
	...
	$search_record = mysqli_fetch_all($select, MYSQLI_ASSOC);
    	...
    	$html .= "<th class='col col-auto ps-2 pe-2 text-uppercase'>ID</th>
				...
				<th class='col col-auto ps-2 pe-2 text-uppercase'>PARENT</th>
				<th class='col col-auto ps-2 pe-2 text-uppercase'>VACCINE</th>
				<th class='col col-auto ps-2 pe-2 text-uppercase'>STATUS/GUARDIAN</th>
				<th class='col col-auto ps-2 pe-2 text-uppercase'>CONTACT NUMBER</th>
				<th class='col col-auto ps-2 pe-2 text-uppercase'>DATE</th>
				<th class='col col-auto ps-2 pe-2 text-uppercase'>TIME</th>
				<th class='col col-auto ps-2 pe-2 text-uppercase'>REASON</th>
				<th class='col col-auto ps-2 pe-2 text-uppercase'>ACTION</th>";
	    ...
	    foreach($search_record as $value){
	    	...
	    	$html.= "<tr>";
	    			...
	    			$html .= "<td class='col col-auto ps-2 pe-2 text-uppercase'>".$value['guardian_name']."</td>";
	    			...
	    			$html .= "<td class='col col-auto ps-2 pe-2 text-uppercase'>".$value['contact_number']."</td>";
	    			$html .= "<td class='col col-auto ps-2 pe-2 text-uppercase'>".$value['appt_date']."</td>";
	    			$html .= "<td class='col col-auto ps-2 pe-2 text-uppercase'>".$value['appt_time']."</td>";
	    			$html .= "<td class='col col-auto ps-2 pe-2 text-uppercase'>".$value['for_reason']."</td>";
	    			if($value['appointment_status'] == 1){
	    			$html .= "<td class='col col-auto ps-2 pe-2 text-uppercase'>";
		    			$html .= "<div class='row'>";
			    			$html .= "<div class='col-lg-4'>";
					    		$html .= ' <a id="send_notif-'.$appt_id.'"  style="cursor:pointer;">
                                                                <i class="fa fa-comment"></i>';
			    			$html .= "</div>";
			    			$html .= "<div class='col-lg-4'>";
					    		$html .= ' <a href="../Admin/admin_more_info.php?id='.$value['cid'].'"  style="cursor:pointer; color:black;">
                                                                <i class="fa fa-eye"></i>';
			    			$html .= "</div>";
			    			$html .= "<div class='col-lg-4'>";
					    		$html .= ' <a id="update_record-'.$appt_id.'"  style="cursor:pointer;">
